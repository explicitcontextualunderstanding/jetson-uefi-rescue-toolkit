# Jetson Orin Nano UEFI Rescue Toolkit & Shell Tutorial

A practical, empirical guide and toolkit for rescuing unbootable NVIDIA Jetson Orin Nano boards via the interactive UEFI Shell, inspecting firmware volumes offline, and repairing FAT32/ESP boot media.
...
├── nsh/                           # Interactive scripts for live UEFI Shell (Shell>)
│   ├── probe_uefi_shell.nsh       # Probes complete command catalog non-destructively (-?)
│   ├── startup.nsh                # Drop-in auto-recovery helper on ESP root
│   ├── stage-grub.nsh             # Copies GRUB + config beside failing L4TLauncher
│   └── boot-kernel-stub.nsh       # EFI stub direct Linux kernel launcher
├── host/                          # Pre-boot forensic & repair tools (Linux / macOS)
│   ├── uefi_boot_verifier.sh      # Comprehensive fail-fast USB/ESP verifier (~1500 lines)
│   ├── diagnose_uefi_boot.py      # Low-level MBR, GPT, and FAT32 boot sector analyzer
│   ├── stage-fat-esp.sh           # Compliant FAT32 ESP + ISO9660 partition provisioner
│   ├── fix_esp_dir_clusters.py    # FAT32 start-cluster (+2 offset) repair script
│   └── check_esp_pe_binaries.py   # Scans disk for valid AArch64 (0xAA64) PE headers
├── firmware/                      # Offline EDK2 firmware analysis
...
└── templates/                     # Known-good templates
    ├── grub.cfg                   # Fallback serial console GRUB config (ttyTCU0,115200)
