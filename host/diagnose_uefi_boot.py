...
def analyze_gpt(device='/dev/sda'):
    ...
    with open(device, 'rb') as f:
        ...
        print(f"Device size: {total_bytes} bytes ({total_bytes/1024/1024/1024:.2f} GB)")
        ...
        for i in range(4):
            ...
            if entry[4] != 0:
                print(f"  MBR Partition {i+1}: type=0x{entry[4]:02x}, LBA={struct.unpack('<I', entry[8:12])[0]}")
        ...
        if hdr[0:8] == b'EFI PART':
            ...
            for i in range(4):
                ...
                name = ''.join(chr(b) if b < 128 else '?' for b in name_bytes[:20] if b != 0)
                print(f"  Entry {i+1}: type={ptype[:16]}... LBA={first_lba}-{last_lba} ({psize*512/1024/1024/1000:.1f}GB) name={name}")
                
                # Check if partition 2 has the right GUID bytes
                if i == 1:
                    esp_bytes = bytes.fromhex('28732ac11ff8d211ba4b00a0c93ec93b')  # Correct mixed-endian
                    msd_bytes = bytes.fromhex('c12a7328f81f11d2ba4b00a0c93ec93b')  # Wrong endianness
                    if entry[0:16] == esp_bytes:
                        print(f"    -> Correct mixed-endian ESP GUID")
                    elif entry[0:16] == msd_bytes:
                        print(f"    -> WRONG: Big-endian GUID (common bug)")
                    else:
                        print(f"    -> Unknown GUID")
        ...
        if backup_hdr[0:8] == b'EFI PART':
            ...
            for i in range(4):
                ...
                name_bytes = entry[56:120]
                name = ''.join(chr(b) if b < 128 else '?' for b in name_bytes[:20] if b != 0)
                print(f"  Entry {i+1}: type={ptype[:16]}... LBA={first_lba} name={name}")
...
def analyze_fat(device='/dev/sda2'):
    ...
    with open(device, 'rb') as f:
        ...
        fat_size = struct.unpack('<I', boot[32:36])[0] if struct.unpack('<H', boot[22:24])[0] == 0 else struct.unpack('<H', boot[22:24])[0]
        num_fats = boot[16]
        print(f"FAT size (sectors): {fat_size}")
        print(f"Number of FATs: {num_fats}")
        
        root_cluster = struct.unpack('<I', boot[46:50])[0]
...
def analyze_iso9660(iso_path='/home/amazon1148/workspace/nano2/jetsoninstaller-r39.2.1-patched-full.iso'):
    """Analyze the ISO9660 filesystem structure."""
    print(f"\n{'='*60}")
    print(f"=== ISO9660 Analysis for {iso_path} ===")
    
    if not os.path.exists(iso_path):
        print("ISO not found at default path, searching...")
        import glob
        isos = glob.glob('/home/amazon1148/workspace/*nano*installer*.iso') + \
               glob.glob('/home/amazon1148/workspace/*installer*.iso') + \
               glob.glob('/home/amazon1148/workspace/*.iso')
        if isos:
            iso_path = isos[0]
            print(f"Found: {iso_path}")
        else:
            print("No ISO found!")
            return
    ...
    with open(iso_path, 'rb') as f:
        f.seek(0, 2)
        iso_size = f.tell()
        print(f"ISO size: {iso_size} bytes ({iso_size/1024/1024/1024:.2f} GB)")
        ...
        print(f"  Volume Space Size: {struct.unpack('>I', pvd[80:84])[0]} sectors")
        print(f"  Volume Set Size: {struct.unpack('>H', pvd[84:86])[0]}")
        print(f"  Volume Seq Num: {struct.unpack('>H', pvd[88:90])[0]}")
        
        # Check if there's padding/zero sectors between PVD and boot record
        f.seek(34816)  # LBA 17
        vd = f.read(2048)
        ...
        if vd[0] == 0x00:  # Boot record
            print(f"  (Boot Record - ISO9660 boot indicator)")
        
        # Check for El Torito Boot Catalog
        f.seek(34816)  # LBA 17 - could be El Torito
        et_data = f.read(2048)
        if et_data[1:6] == b'CD001':
            print(f"\n--- El Torito Boot Entry (LBA 17) ---")
            print(f"  Type: {et_data[0]}")
            print(f"  Magic: {et_data[1:6]}")

def check_iso_files(iso_path='/home/amazon1148/workspace/nano2/jetsoninstaller-r39.2.1-patched-full.iso'):
    """List key files in the ISO9660 filesystem."""
    print(f"\n{'='*60}")
    print(f"=== ISO9660 File Check ===")
    ...
    try:
        ...
        for path in files_to_check:
            # Try path as ISO would store it (mixed case)
            for test_path in [path, path.lower(), path.upper()]:
                try:
                    iso_filename = test_path.replace('/', '\\')[:1] + test_path.replace('/', '\\')[1:].upper()
                    iso.get_file_from_iso(iso_filename.encode('utf-16be'), os_path=path)
                    print(f"  [FOUND] {path}")
                    break
                except:
                    pass
        ...
        print("pycdlib not installed, trying file mount...")
        
        # Try reading the ISO as a filesystem
        import subprocess
        try:
            result = subprocess.run(['isoinfo', '-i', iso_path, '-l'], 
...
if __name__ == '__main__':
    ...
    if os.path.exists('/dev/sda'):
        analyze_gpt('/dev/sda')
        analyze_fat('/dev/sda2')
    else:
        print("USB (/dev/sda) not connected")
    
    # Check ISO
    iso_candidates = [
        '/mnt/nvme/jetsoninstaller-r39.2.1-patched-full.iso',
        '/home/amazon1148/workspace/nano2/jetsoninstaller-r39.2.1-patched-full.iso',
        '/home/amazon1148/workspace/jetsoninstaller-r39.2.1-patched-full.iso',
    ]
    ...
    if iso_path:
        analyze_iso9660(iso_path)
        check_iso_files(iso_path)
    else:
        print("\nISO not found, checking common locations...")
        import glob
        isos = glob.glob('/home/amazon1148/**/*.iso', recursive=True) + \
               glob.glob('/mnt/**/*.iso', recursive=True)
        if isos:
            for iso in isos[:3]:
                print(f"  Found: {iso}")
        else:
